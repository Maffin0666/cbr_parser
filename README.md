...
P.S.: В данном проекте не предоставлены необходимые файлы docker - нужно создать их самостоятельно
...
# Часто выдавал ошибку, даже если отключать авто-перезагрузку
python manage.py runserver --noreload

# Поэтому рекомендую другой вариант сервера (более стабилен для многопоточности)
...
## Результат
После успешного запуска система будет:
- Предоставлять доступ к админ-панели
- Обновлять курсы валют и данные банков следуя заданному расписанию
- Хранить данные в PostgreSQL (банки, курсы и логи)

Их всегда можно просмотреть при помощи запросов, например:
```sql
# для курсов
SELECT * FROM cbr_parser_currencyrate;
# для банков
SELECT * FROM cbr_parser_bank;
# для логов
SELECT * FROM cbr_parser_tasklog;
```
